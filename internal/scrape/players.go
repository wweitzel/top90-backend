...
func (s *Scraper) linkPlayerWithApifootball(redditPostTitle string, fixtureId int) (db.Player, apifootball.Event, error) {
	...
	if err != nil {
		return db.Player{}, apifootball.Event{}, err
	}

	playerExists, err := s.dao.PlayerExists(event.Player.ID)
	if err != nil {
		return db.Player{}, apifootball.Event{}, fmt.Errorf("failed checking if player exists in db: %v", err)
	}

	if playerExists {
		player, err := s.dao.GetPlayer(event.Player.ID)
		if err != nil {
			return db.Player{}, apifootball.Event{}, fmt.Errorf("failed getting existing player from db")
		}
	...
	if err != nil {
		return db.Player{}, apifootball.Event{}, fmt.Errorf("failed getting player from apifootball")
	}
	insertedPlayer, err := s.dao.UpsertPlayer(player)
	if err != nil {
		return db.Player{}, apifootball.Event{}, fmt.Errorf("failed upserting player into db")
	}
